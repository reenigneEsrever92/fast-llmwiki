...
description: A single okf-cli crate that starts okf-server, okf-gui, or both together.
status: stable
tags: [dev, cli]
owner: human:felix
---
...
- A new `okf-cli` crate provides the primary `okf` binary as the unified entry point.
...
- The server router (`okf_server::api::router`) and the GUI router
  (`okf_gui::ssr::router`) MUST remain in their current crates; `okf-cli` only
...
- The `okf-server` crate's binary MUST be renamed from `okf` to `okf-server` so its
